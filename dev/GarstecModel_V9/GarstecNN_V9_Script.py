...
from pytorch_lightning.callbacks import ModelCheckpoint, LearningRateMonitor, Timer, LearningRateFinder

timer_callback = Timer(duration="00:06:00:00")  # dd:hh:mm:ss
...
checkpoint_callback = ModelCheckpoint(
    monitor='val_loss',
    dirpath=save_dir,
    filename='best_model_v9-{epoch:02d}-{val_loss:.8f}',
    save_top_k=1,
    mode='min'
)
...
lr_finder_callback = LearningRateFinder(
    ...
    update_attr=True,
    attr_name='lr'  # This will update the learning rate in the model
)
...
lr_finder_trainer.fit(model, data_module)

# Get the suggested learning rate and update model
suggested_lr = model.lr
...
print(f"Training model with learning rate: {suggested_lr}")
main_trainer.fit(model, data_module)

# Save the final model
final_checkpoint_path = checkpoint_callback.best_model_path
main_trainer.save_checkpoint(final_checkpoint_path)
