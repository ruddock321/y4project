...
print("SCRIPT FOR RESIDUAL NET WITH 256 NEURONS - 5 day runtime")

timer_callback = Timer(duration="00:120:00:00")  # dd:hh:mm:ss
...
# Pre-activation residual block without dropout
...
# Reduced architecture LightningModule
class GarstecNet(LightningModule):
    def __init__(self, input_dim, output_dim, lr=1e-3):
        ...
        # Input layer: project input_dim to 256 units
        self.fc_in = nn.Linear(input_dim, 256)
        self.relu = nn.ReLU()
        
        # Three stacked pre-activation residual blocks at 256 dimensions
        self.res_block1 = PreActivationResidualBlock(256)
        self.res_block2 = PreActivationResidualBlock(256)
        self.res_block3 = PreActivationResidualBlock(256)
        
        # Output layer: project from 256 units to output_dim
    ...
    def configure_optimizers(self):
        # Using AdamW for potentially more stable convergence
...
checkpoint_callback = ModelCheckpoint(
    monitor='val_loss',
    dirpath=save_dir,
    filename='best_model_v9-3---{epoch:02d}-{val_loss:.8f}',
    save_top_k=1,
    mode='min'
)
