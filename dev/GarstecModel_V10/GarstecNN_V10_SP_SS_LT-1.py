...
class GarstecNet(LightningModule):
    def __init__(self, input_dim, output_dim, loss_weights, lr=1e-3, min_lr=1e-8, T_0=100, T_mult=2):
        super().__init__()
        self.save_hyperparameters()
        self.lr = lr
        self.min_lr = min_lr
        self.T_0 = T_0  # Initial period for warm restarts
        self.T_mult = T_mult  # Multiplicative factor for increasing restart period
    ...
    def configure_optimizers(self):
        ...
        # Use CosineAnnealingWarmRestarts instead of CosineAnnealingLR
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer,
            T_0=self.T_0,   # Initial restart period
            T_mult=self.T_mult,  # Restart period growth factor
...
# Modify continued training settings
model.min_lr = 1e-8  # Minimum learning rate for cosine scheduler

# Callbacks
checkpoint_callback = ModelCheckpoint(
    monitor='val_loss',
    dirpath=save_dir,
    filename='continued_model_v10_SP_SS-SGD-CAWR---{epoch:02d}-{val_loss:.8f}',
    save_top_k=1,
    mode='min'
)
