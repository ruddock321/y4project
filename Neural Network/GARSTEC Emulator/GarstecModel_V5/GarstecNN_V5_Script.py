...
dataloader = DataLoader(dataset, batch_size=2**11, shuffle=True)  
...
class GarstecNet(nn.Module):
    def __init__(self):
        ...
        # Output layer
        self.dense6 = nn.Linear(128, 8)  # Assuming 8 output features
        
    def forward(self, x):
        x = torch.relu(self.dense1(x))
        x = self.dropout1(x)  # Apply dropout to prevent overfitting
...
if best_model_wts is not None:
    ...
    torch.save({
        ...
        'loss': best_test_loss,
    }, os.path.join(save_dir, 'best_model_v5.pth'))  # Save model checkpoint
