...
timer_callback = Timer(duration="00:00:03:00")  # dd:hh:mm:ss
...
class GarstecNet(LightningModule):
    def __init__(self, input_dim, output_dim, lr=1e-3, loss_weights=None):
        super().__init__()
        ...
        if loss_weights is None:
            # Weights for [T_eff, L, dnu, numax, feh]
            self.loss_weights = torch.tensor([70.0, 0.1, 0.1, 0.5, 0.01], device=self.device)
        else:
    ...
    def custom_loss(self, y_pred, y):
        ...
        weights = self.loss_weights.to(y.device)
...
loss_weights = [70.0, 0.1, 0.1, 0.5, 0.01]

# Define model and trainer
input_dim = X_train.shape[1]
output_dim = y_train.shape[1]
model = GarstecNet(input_dim=input_dim, output_dim=output_dim, lr=1e-3, loss_weights=loss_weights)

# Callbacks
checkpoint_callback = ModelCheckpoint(
    monitor='val_loss',
    dirpath=save_dir,
    filename='best_model_v10---{epoch:02d}-{val_loss:.8f}',
    save_top_k=1,
    mode='min'
)
