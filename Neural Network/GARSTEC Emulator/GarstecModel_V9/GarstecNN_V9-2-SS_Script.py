...
class GarstecNet(LightningModule):
    def __init__(self, input_dim, output_dim, lr=1e-3):
        ...
        # Input layer: project input_dim to 128 units
        self.fc_in = nn.Linear(input_dim, 128)
        self.relu = nn.ReLU()
        
        # Three stacked pre-activation residual blocks at 128 dimensions
        self.res_block1 = PreActivationResidualBlock(128)
        self.res_block2 = PreActivationResidualBlock(128)
        self.res_block3 = PreActivationResidualBlock(128)
        
        # Output layer: project from 128 units to output_dim
...
checkpoint_callback = ModelCheckpoint(
    monitor='val_loss',
    dirpath=save_dir,
    filename='best_model_v9-2-SS---{epoch:02d}-{val_loss:.8f}',
    save_top_k=1,
    mode='min'
)
