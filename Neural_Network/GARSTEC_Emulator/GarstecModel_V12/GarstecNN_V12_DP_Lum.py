...
class GarstecDataModule(LightningDataModule):
    def __init__(self, X_train, X_test, y_train, y_test, batch_size=2**14):
        super().__init__()
...
class GarstecNet(LightningModule):
    def __init__(self, input_dim, output_dim, loss_weights, lr=1e-3, 
        ...
        # Change model weights to float64
...
loss_weights = [70, 0.01, 0.1, 0.5/3090, 0.01]  # Division by 3090
...
checkpoint_callback = ModelCheckpoint(
    monitor='val_loss',
    dirpath=save_dir,
    filename='best_model_noLogDP_v12_Lum---{epoch:02d}-{val_loss:.8f}',
    save_top_k=1,
    mode='min'
)
