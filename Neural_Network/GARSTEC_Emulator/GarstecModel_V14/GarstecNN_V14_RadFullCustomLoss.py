...
save_dir = r'/rds/projects/d/daviesgr-m4massloss/GarstecNN_V14'
...
class GarstecNet(LightningModule):
    def __init__(self, input_dim, output_dim, lr=1e-3):
        ...
        # Define loss weights
        self.loss_weights = torch.tensor([70, 0.1, 0.5/3090, 0.01, 0.01])
    ...
    def set_output_stats(self, mean, scale):
        """Set the mean and scale for the output variables"""
        self.output_mean = torch.tensor(mean, dtype=torch.float32)
        self.output_scale = torch.tensor(scale, dtype=torch.float32)
    ...
    def custom_loss(self, y_pred, y_true):
        ...
        # Convert log outputs back to linear scale for the first 3 columns
        # Create a mask for log-transformed features (first 3 columns)
        log_mask = torch.zeros_like(y_pred_original)
        log_mask[:, :3] = 1.0  # First 3 columns were log-transformed
        ...
        # Sum across features and average across batch
        return torch.mean(torch.sum(weighted_squared_diff, dim=1))
...
# Set output scaler stats in the model for inverse transformation
model.set_output_stats(output_scaler.mean_, output_scaler.scale_)

# Callbacks
checkpoint_callback = ModelCheckpoint(
    monitor='val_loss',
    dirpath=save_dir,
    filename='best_model_v14-RadFullCustomLoss---{epoch:02d}-{val_loss:.8f}',  # Changed to RadFullMSE
    save_top_k=1,
    mode='min'
)
