...
# Set PyTorch to use double precision by default
torch.set_default_dtype(torch.float64)

timer_callback = Timer(duration="00:00:05:00")  # dd:hh:mm:ss
...
class GarstecNet(LightningModule):
    def __init__(self, input_dim, output_dim, loss_weights, lr=1e-3):
        ...
        self.lr = lr
        self.register_buffer("loss_weights", torch.tensor(loss_weights, dtype=torch.float64))
    ...
    def training_step(self, batch, batch_idx):
        x, y = batch
        x, y = x.to(self.device, dtype=torch.float64), y.to(self.device, dtype=torch.float64)
        y_pred = self(x)
...
target_uncertainties = [70.0, 0.1, 0.1, 0.5, 0.01]

std_devs = output_scaler.scale_  

# Calculate final weights for your loss function
final_weights = []
for i, (uncertainty, std_dev) in enumerate(zip(target_uncertainties, std_devs)):
    if i < 4:  # For log-transformed features (Teff, L, dnu, numax)
        # First convert to log space using typical values
        typical_value = [4953.89, 55.89, 3.97, 0.01][i]  # Using medians
        log_uncertainty = uncertainty / (typical_value * np.log(10))
        
        # Then adjust for standardization
        weight = log_uncertainty / std_dev
    else:  # For FeH (not log-transformed)
        weight = uncertainty / std_dev
        
    final_weights.append(weight)

# Use these as your loss weights
...
checkpoint_callback = ModelCheckpoint(
    monitor='val_loss',
    dirpath=save_dir,
    filename='best_model_v10_double-SS---{epoch:02d}-{val_loss:.8f}',
    save_top_k=1,
    mode='min'
)
...
trainer = Trainer(
    ...
    precision='64-true',  # Set PyTorch Lightning to use 64-bit precision
)
